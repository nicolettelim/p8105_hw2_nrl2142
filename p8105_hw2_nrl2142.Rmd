...
There are a total of `r length(missingzipcode_df)` zip codes in `zipcode_df` that do not appear in `zori_df`. For example `r paste(head(missingzipcode_df, 5), collapse = ",")`. Potential reasons for this are 1. Some zipcodes may be primarily commercial or industrial and thus have to relevance to Zillow's rental index in the `zori_df` dataset. 2. Data in `zipcode_df` was recorded in 2007 and some zipcodes could have been removed or changed, making it irrelevant to the `zori_df` dataset that captures rent index from 2015 to 2024. 
